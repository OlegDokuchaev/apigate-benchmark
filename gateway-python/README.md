...
`../gateway-apigate/` (Rust), `../gateway-kong/` (Kong OSS), and
`../gateway-apisix/` (APISIX). Bare ASGI,
...
- `401` — `/my-items` without `Authorization`, with an invalid token, or when `/verify` cannot yield a valid identity.
- `502` — data upstream returned an unexpected network error.
- `503` — the gateway has not finished startup yet.
- `504` — data upstream timed out.
...
| `AIOHTTP_TCP_KEEPALIVE_IDLE` | `30`                            | Socket-level TCP keepalive idle before first probe where the OS exposes `TCP_KEEPIDLE` / `TCP_KEEPALIVE`. Mirrors apigate `TCP_KEEPALIVE=30s` and the compose `tcp_keepalive_time=30` used by Kong/APISIX. |
| `GRANIAN_HTTP1_KEEP_ALIVE`  | `true`                          | Adds Granian `--http1-keep-alive` so client -> gateway HTTP/1.1 connections stay persistent. |
| `UPSTREAM_CONNECT_TIMEOUT`  | `3.0`                           | Data upstream connect (s).                       |
| `UPSTREAM_TOTAL_TIMEOUT`    | `10.0`                          | Data upstream total (s).                         |
| `AUTH_CONNECT_TIMEOUT`      | `3.0`                           | Auth upstream connect (s) — aligned with data and the other gateways. |
...
Both entry points read `GRANIAN_HTTP1_KEEP_ALIVE` and pass
`--http1-keep-alive` when it is `true`, so inbound client -> gateway HTTP/1.1
connections stay persistent under k6.
...
- **One shared `aiohttp.ClientSession`.** Created during ASGI
  ...
  2048 cumulative, matches Kong / APISIX / apigate), `keepalive_timeout=120s`
  ...
  `socket_factory` enables TCP `SO_KEEPALIVE` and sets the idle probe delay
  to 30s where the OS exposes it. DNS is cached.
...
- **Split timeouts.** The proxy path uses `sock_read=None` because the
  ...
  `sock_read=AUTH_TOTAL_TIMEOUT` to catch a stuck auth faster, then collapses
  every auth failure to `401` like the other gateways. `total` still bounds
  both paths.
...
- **Granian `--http1-keep-alive`.** Enabled by
  `GRANIAN_HTTP1_KEEP_ALIVE=true` for client -> gateway sockets. aiohttp
  separately controls gateway -> upstream keep-alive with
